...
# If this file is run directly, start the Flask app
if __name__ == "__main__":
    port = int(os.environ.get('PORT', 5000))
    debug_mode = os.environ.get('FLASK_DEBUG', 'True').lower() == 'true'
    
    # Check if SSL certificate paths are provided for HTTPS
    cert_path = os.environ.get('SSL_CERT_PATH')
    key_path = os.environ.get('SSL_KEY_PATH')
    
    ssl_context = None
    if cert_path and key_path and os.path.exists(cert_path) and os.path.exists(key_path):
        logger.info(f"Starting server with HTTPS support on port {port}")
        ssl_context = (cert_path, key_path)
    else:
        logger.info(f"Starting server with HTTP only on port {port} (no SSL certificates found)")
        logger.info("To enable HTTPS, set SSL_CERT_PATH and SSL_KEY_PATH environment variables")
    
    # Log startup information
    logger.info(f"Email Verifier API starting up (Debug mode: {debug_mode})")
